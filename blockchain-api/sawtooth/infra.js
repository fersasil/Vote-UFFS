...
function handlerInfo(){
  const familyName = 'uffsOnlineVotations';
  return {
...
function calculateVoteAddress(payload) {
  const urlAdress = handlerInfo().prefix + getAddress(payload.ellectionName, 20) + getAddress(payload.userNumber, 20) + getAddress(payload.address, 24);
...
function sendToSawtoothApi(batchBytes) {
  request({
      ...
      if (error) {
        ...
        const res = new Buffer.from(response.body, 'base64').toString()
  ...
  request({
      ...
      if (error) {
        ...
        const decodedInfo = items.map((item) => {
          return JSON.parse(new Buffer.from(item.data, 'base64').toString());
        });
